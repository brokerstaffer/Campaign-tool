-- 054 — one page of a campaign's leads.
--
...
LANGUAGE sql STABLE AS $$
      ...
      CASE
        WHEN EXISTS (SELECT 1 FROM replies r
                      WHERE r.campaign_id = cl.campaign_id AND r.lead_id = cl.lead_id
                        AND r.is_bounce_notification)                     THEN 'bounced'
        WHEN EXISTS (SELECT 1 FROM replies r
                      WHERE r.campaign_id = cl.campaign_id AND r.lead_id = cl.lead_id
                        AND r.tracked_reply AND NOT r.is_bounce_notification
                        AND r.sentiment = 'positive')                     THEN 'positive'
        WHEN EXISTS (SELECT 1 FROM replies r
                      WHERE r.campaign_id = cl.campaign_id AND r.lead_id = cl.lead_id
                        AND r.tracked_reply AND NOT r.is_bounce_notification) THEN 'replied'
        WHEN cl.step_reached >= (SELECT COUNT(*) FROM sequence_steps ss
                                  WHERE ss.campaign_id = cl.campaign_id
                                    AND NOT ss.is_variant)                THEN 'completed'
        ELSE 'contacted'
  ...
  SELECT
    ...
    (SELECT COUNT(*) FROM replies r
      WHERE r.campaign_id = p_campaign_id AND r.lead_id = p.lead_id
        AND r.tracked_reply AND NOT r.is_bounce_notification),
    (SELECT COUNT(*) FROM replies r
      WHERE r.campaign_id = p_campaign_id AND r.lead_id = p.lead_id
        AND r.tracked_reply AND NOT r.is_bounce_notification
...
INSERT INTO schema_migrations (version) VALUES ('054_campaign_lead_rows')
